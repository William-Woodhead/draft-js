...
function getUpdatedSelectionState(
  ...
  var anchorTree = editorState.getBlockTree(anchorBlockKey);

  if (!anchorTree) {
    return selection;
  }

  var anchorLeaf = anchorTree
    .getIn([
  ...
  var focusTree = editorState.getBlockTree(focusBlockKey);

  if (!focusTree) {
    return selection;
  }

  var focusLeaf = focusTree
    .getIn([
